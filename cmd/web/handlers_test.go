...
func TestPing(t *testing.T) {
	...
	if statusCode != http.StatusOK {
		t.Errorf("want %q; got %q", http.StatusOK, statusCode)
	}
...
func TestSignupUser(t *testing.T) {
	...
	log.Println("Before GET signup...")
	_, _, body := ts.get(t, "/user/signup")
	log.Println("After GET signup...")
	...
	tests := []struct {
		...
		{"Valid submission", "Nacho", "nacho@gmail.com", "1234567890", csrfToken, http.StatusSeeOther, []byte("")},
		...
		{"Invalid CSRF Token", "", "", "", "wrongToken", http.StatusBadRequest, []byte("Bad Request")},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			form := url.Values{}
